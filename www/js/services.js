...
.factory("userService", ['localStorageService', function(localStorageService) {
    ...
    return {
        init: function(users) {
            var i = 0, length = 0, userId = new Array();
            if (users) {
                length = users.length;
                for (; i < length; i++) {
                    userId[i] = {
                ...
                for (i = 0; i < length; i++) {
                    localStorageService.update("users_" + userId[i].id, users[i]);
                }
            }
        },
        getUsers: function() {
            /*return $http.get("data/json/friends1.json").then(function(response) { //https://randomuser.me/api/?results=10
            ...
            if (userId) {
                ...
                for (; i < length; i++) {
                    user = localStorageService.get("users_" + userId[i].id);
                    if(user){
                        users.push(user);
                    }
                }
        ...
        getUserById: function(id) {
            return localStorageService.get("users_" + id)
        }/*,
